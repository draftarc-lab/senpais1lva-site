...
const picks = [
  ...
  ...summerCoverage.map((show) => ({
    title: show.canonicalTitle,
    type: show.contentType === "donghua" ? "Donghua" : "Anime",
    mood: show.genres.slice(0, 2).join(" · "),
    tags: ["Covered by SenpaiS1lva", ...show.genres],
...
export default function RecommendationExplorer() {
  ...
  return <>
    <section className="container recommendation-tools" aria-label="Recommendation filters">
      ...
      <div>{filters.map((item) => <button aria-pressed={filter === item} className={filter === item ? "active" : ""} type="button" key={item} onClick={() => setFilter(item)}>{item}</button>)}</div>
    </section>
