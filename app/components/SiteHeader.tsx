...
const links = [
  ...
  { href: "/senpai-notes", label: "Senpai Notes" },
  ...
  return (
    <>
      ...
      <nav className="nav container" aria-label="Primary navigation">
        ...
        <Link className={`nav-cta ${pathname === "/work-with-me" ? "active" : ""}`} href="/work-with-me">Work With Me</Link>
        ...
        {open && <div id="mobile-navigation" className="mobile-navigation">{links.map((link) => <Link key={link.href} href={link.href} onClick={() => setOpen(false)}>{link.label}</Link>)}<Link href="/work-with-me" onClick={() => setOpen(false)}>Work With Me</Link><Link href="/courplay" onClick={() => setOpen(false)}>Courplay</Link><Link href="/silvas-circle" onClick={() => setOpen(false)}>Silva’s Circle</Link></div>}
      </nav>
